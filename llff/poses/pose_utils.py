...
def load_colmap_data(realdir):
    ...
    return poses, pts3d, perm, imdata


def save_poses(basedir, poses, pts3d, perm, imdata):
    pts_arr = []
    vis_arr = []
    for k in pts3d:
        pts_arr.append(pts3d[k].xyz)
        cams = [0] * poses.shape[-1]
        image_id_to_idx = {list(sorted(imdata.keys()))[i]: i for i in range(len(imdata))}
        for ind in pts3d[k].image_ids:
            if ind in image_id_to_idx:
                idx = image_id_to_idx[ind]
                if idx < len(cams):
                    cams[idx] = 1
            else:
                print(f'Warning: Image ID {ind} not found in image data')
        vis_arr.append(cams)
...
def gen_poses(basedir, match_type, factors=None):
    ...
    poses, pts3d, perm, imdata = load_colmap_data(basedir)
    
    save_poses(basedir, poses, pts3d, perm, imdata)
